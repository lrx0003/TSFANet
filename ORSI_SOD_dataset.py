...
class ORSI_SOD_dataset(data.Dataset):
    def __init__(self, root, mode, aug=False):
        ...
        self.prefixes = [line.strip() for line in open(os.path.join(root.replace(mode, ''), mode+'.txt'))]

        self.image_paths = [os.path.join(root, 'image', prefix + '.jpg') for prefix in self.prefixes]
        self.label_paths = [os.path.join(root, 'label', prefix + '.png') for prefix in self.prefixes]
        self.edge_paths = [os.path.join(root, 'edge', prefix + '.png') for prefix in self.prefixes]
