...
def getsmaps(dataset_name, model_Type):
    ...
    dataset_root = "./data/" + dataset_name + "/test/"
    ...
    if model_Type == "HFANet_R":
        ...
        if dataset_name == "ORSSD":
            net.load_state_dict(torch.load("./HFANet_R_weights/ORSSD_weights.pth", map_location='cuda:0'))
        elif dataset_name == "EORSSD":
            net.load_state_dict(torch.load("./HFANet_R_weights/EORSSD_weights.pth", map_location='cuda:0'))
        elif dataset_name == "ORS_4199":
            net.load_state_dict(torch.load("./HFANet_R_weights/ORS_4199_weights.pth", map_location='cuda:0'))
    
    elif model_Type == "HFANet_V":
        net = Net_V().cuda().eval()  
        if dataset_name == "ORSSD":
            net.load_state_dict(torch.load("./HFANet_V_weights/ORSSD_weights.pth", map_location='cuda:0'))
        elif dataset_name == "EORSSD":
            net.load_state_dict(torch.load("./HFANet_V_weights/EORSSD_weights.pth", map_location='cuda:0'))
        elif dataset_name == "ORS_4199":
            net.load_state_dict(torch.load("./HFANet_V_weights/ORS_4199_weights.pth", map_location='cuda:0'))
    
    net.eval()


    for image, label, name in tqdm(test_loader):
